...
int parseUpdate(int actual, int argc, char *argv[], Plow &plow)
{
	...
	while(actual < argc && argv[actual][0] != '-' && argv[actual][0]
			!= '+') {
		if(argv[actual][1] != 0) {
...
int parseLongOption(int actual, int argc, char *argv[], Plow &plow)
{
	...
	if(strcmp(value, "v") == 0 || strcmp(value, "version") == 0) {
		...
		if(argv[++actual] == 0) {
			throw PlowException("parseLongOption",
					"missing sql statement for option '--query'", USAGE);
		}
		...
		if(argv[++actual] != 0) {
			...
			throw PlowException("parseLongOption",
					"missing argument for option '--list'", USAGE);
		}
	}
		...
		if(argv[++actual] != 0) {
			...
			if (argv[++actual] != 0) {
				...
				sprintf(query, "SELECT %1$s FROM tbl_%1$s ORDER BY %1$s;",
						quotedTable);
			...
			throw PlowException("parseLongOption",
					"missing argument for option '--find'", USAGE);
		}
	}
